...
% Last Modified by GUIDE v2.5 14-Oct-2010 11:31:39
...
if(btn(1)==1&&btn(2)==1)
    ...
    guidata(hObject, handles);
    datawrite(x1,x2,x3,x4,x5,x6,x7);
    Fs=handles.wavfile.feq;
    if(xx(2)>xx(1))
        Left=xx(1);
        Right=xx(2);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
wavplay(handles.wavfile.data1,handles.wavfile.feq);
...
if(handles.flag.oplevel>1)
    [x1,x2,x3,x4,x5,x6,x7]=dataread(handles.flag.oplevel,handles.flag.opL);
